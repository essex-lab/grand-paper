...
# Load PDB
pdb = PDBFile('../equil/bpti-nvt2.pdb')

# Add ghost waters - not really necessary as they aren't used...
...
# Use the GCMC mover to track the waters in the sphere - not using this to do any sampling!
...
for i in range(10000):
    ...
    if (i+1) % 20 == 0:
        # Making sure the gcmc_mover has the updated positions to write out the value of N
        state = simulation.context.getState(getPositions=True, enforcePeriodicBox=True)
        gcmc_mover.positions = state.getPositions(asNumpy=True)
        simulation.context.setPositions(gcmc_mover.positions)
        gcmc_mover.updateGCMCSphere(state)
